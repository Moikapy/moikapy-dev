...
- [[origen-integration]] — Origen (@moikapy/origen) is the LLM agent engine used for AI routes and community insights. callOrigen() for suggest, streamOrigen() for format with continuation harness. Model: openrouter/free (auto best-free-model). thinkingLevel: "off" to avoid thinking token waste. Community insights cron calls Origen for content advice (what to write next). Config in src/lib/origen.ts. No origen-zero needed — plain OrigenTool (TypeScript) for D1 queries.
- [[post-card]] — Blog post card component. Shows title, date, reading time, excerpt, and tags. Cover image uses capybara-256.webp as default fallback when no coverImage is set (centered with padding on warm bg-muted). Title uses font-heading (Lora serif). Badges use warm gold hover (primary/10). Card has hover:border-primary/50 and hover:shadow-md transition.
...
- [[warm-editorial-theme]] — The blog uses a warm editorial design theme instead of cold monochrome. Light mode: cream background (#faf8f5), warm brown text (#2c2420), gold primary (#8b6914). Dark mode: warm near-black (#1a1614), cream text (#f5f0e8), gold primary (#e8c47c). Headings use Lora serif (font-heading CSS var), body uses Geist Sans. Badge hovers use warm gold (hover:bg-primary/10 hover:text-primary). Post cards default to capybara-256.webp when no cover image. Applied to: globals.css (CSS custom props), layout.tsx (Lora font import), all page headings (font-heading class), PostCard, TrendingThisWeek, TopicSpotlight, RelatedPosts.
...
*Last updated: 2026-05-18 • 34 pages total*
